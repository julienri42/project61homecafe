...
<head>
    ...
    <title>สารสนเทศยอดขาย(ออนไลน์)</title>
    ...
    <div id="wrapper">
        <div class="container-fluid">
            ...
            <div class="text-center fw-bold lh-1 border-top border-bottom border-dark">
                    ...
                    <span>สารสนเทศยอดขาย(ออนไลน์)  <?php
                    if(!empty($_GET['id']))
                    {
            ...
            <table class="table table-striped table-bordered">
                ...
                <?php
                    if(empty($_GET['id']))
                    {
                        ...
                        $sql = "SELECT 	order_id,order_date_added,order_total,order_discount,order_service_total,user_id FROM tb_order
                        ...
                        $month = $_GET['id'];
                        $sql = "SELECT 	order_id,order_date_added,order_total,order_discount,order_service_total,user_id FROM tb_order
                        WHERE order_status != 'พนักงาน' AND order_date_added LIKE '$month%'  AND order_status!='การสั่งซื้อหมดอายุ' AND order_status!='รอการชำระเงิน'
                        ORDER BY order_date_added DESC
                                    ";
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = $result->fetch_array()) {
                            ...
                            echo "<td>" . number_format($row['order_total'] - $row['order_discount']+$row['order_service_total'], 2) . "</td>";
                            if ($row['user_id'] == "0") {
                                echo "<td> - </td>";
                            } else {
